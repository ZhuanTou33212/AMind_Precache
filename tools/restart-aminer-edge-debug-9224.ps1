...
$running = Get-CimInstance Win32_Process -Filter "Name = 'msedge.exe'" |
  Where-Object { $_.CommandLine -like '*annot-aminer-hotkeys-edge-profile-2*' }
...
$arguments = "--remote-debugging-port=$DebugPort --user-data-dir=$EdgeProfile --disable-extensions-except=$extensionList --load-extension=$extensionList --disable-background-mode --no-first-run --new-window $StartUrl"

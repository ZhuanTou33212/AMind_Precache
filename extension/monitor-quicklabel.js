(() => {
  ...
  function clickOptionFor(groupId, value) {
    ...
    // Robust click: both native click() and MouseEvent for React compatibility
    best.clickable.click();
    try { best.clickable.dispatchEvent(new MouseEvent('click', { bubbles: true, cancelable: true, view: window })); } catch(e) {}
    try { best.clickable.dispatchEvent(new MouseEvent('mousedown', { bubbles: true, cancelable: true, view: window })); } catch(e) {}
    try { best.clickable.dispatchEvent(new MouseEvent('mouseup', { bubbles: true, cancelable: true, view: window })); } catch(e) {}
    lastClickedEl = best.clickable;
  ...
  function verifyLabelApplied(node) {
    // Check if element or its parent wrapper shows selected/checked state
    var el = node;
    for (var d = 0; d < 4 && el; d++) {
      ...
      if (el.tagName === 'INPUT' && (el.type === 'radio' || el.type === 'checkbox') && el.checked) return true;
      el = el.parentElement;
    }
    return false;
  }
  ...
  async function applyLabelsToPage(labels) {
    ...
    for (var i = 0; i < values.length; i++) {
      var item = values[i];
      var applied = false;
      // Try up to 3 clicks per label
      for (var attempt = 0; attempt < 3; attempt++) {
        var ok = clickOptionFor(item.groupId, item.value);
        if (!ok) { missing.push(item.value); break; }
        // Verify label was applied using the element that was just clicked
        if (lastClickedEl && verifyLabelApplied(lastClickedEl)) {
          trace('applyLabelsToPage 已确认 ' + item.groupId + '/' + item.value + ' (attempt ' + (attempt+1) + ')');
          applied = true;
          break;
        }
        trace('applyLabelsToPage 重试 ' + item.groupId + '/' + item.value + ' (attempt ' + (attempt+1) + ' not verified)');
        await new Promise(function(r) { setTimeout(r, 80); });
      }
      if (!applied && !missing.includes(item.value)) {
        missing.push(item.value);
      }
      await new Promise(function(r) { setTimeout(r, 80); });
    }
    if (missing.length) {
      trace('applyLabelsToPage FAILED 未确认: ' + missing.join(', '));
      throw new Error('找不到网页选项: ' + missing.join('_'));
    }
    // All labels confirmed, wait 300ms then allow submit
    await new Promise(function(r) { setTimeout(r, 300); });
    trace('applyLabelsToPage 完成，全部 ' + values.length + ' 个标签确认成功');
  }
  ...
  async function handleSubmitRequests() {
    ...
    var labelCount = labelValues(record.labels||{}).length;
    if (labelCount > 1) console.log('[quicklabel] 多标签图片 Q' + questionNum + ': 共 ' + labelCount + ' 个标签');
    submitBusy = true;
    try {
      ...
      // Signal to monitor-main.js (MAIN world) to skip redundant key mapping
      document.documentElement.setAttribute('data-aminer-last-submit', Date.now().toString());
      setTimeout(function() { submitBusy = false; }, 1200);
    }
  }
